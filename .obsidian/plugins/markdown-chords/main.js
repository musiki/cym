...
// node_modules/markdown-it-chords/index.js
var require_markdown_it_chords = __commonJS({
  "node_modules/markdown-it-chords/index.js"(exports, module2) {
    "use strict";
    var CHORD_TEST = /^\[[A-G][b#♭♯]*(?:M|Δ|[Mm]aj|m|[Mm]in|-|–|[Dd]im|o|°|ø|[Aa]ug|\+|[Ss]usp?|[Aa]dd)?(?:1?[\d])?(?:(?:[\(\/,]?(?:[-–+Δob#♭♯]|[Mm]aj|[Mm]in|[Ss]usp?)?[0-9]+\)?)*)?(?:\/[A-G][b#♭♯]*)?(?:\|[XxOo\d,\(\)]{3,})?\]/;
    var CHORD_REGEX = /^\[([A-G])([♭♯]*)(M|Δ|[Mm]aj|m|[Mm]in|‑|[Dd]im|°|ø|[Aa]ug|\+|[Ss]usp?|[Aa]dd)?(1?[\d])?((?:[\(\/,]?(?:[‑+Δ°♭♯]|[Mm]aj|[Mm]in|[Ss]usp?)?[0-9]+\)?)*)?(\/[A-G][♭♯]*)?(\|[XxOo\d,\(\)]{3,})?\]/;
    var EXTENDED_REGEX = /(?:[\(\/,1]*(?:[‑+Δ°♭♯]|[Mm]aj|[Mm]in|[Ss]usp?)?[0-9]+\)?)*/;
    var DIAGRAM_REGEX = /^\[(?:[XxOo\d,\(\)]{3,})\]/;
    function chords(state, silent) {
      let tail, chordMatch, chordSplit, chord, diagram, extended, token, classes, pos = state.pos;
      if (state.src.charCodeAt(pos) !== 91)
        return false;
      ...
      if (chordMatch) {
        ...
        chord = (chordSplit[0] + "]").replace(/b/g, "\u266D").replace(/#/g, "\u266F").replace(/o/g, "\xB0").replace(/[-–]/g, "\u2011").match(CHORD_REGEX);
    ...
    function parseDiagram(diagram) {
      if (!diagram)
        return false;
      const fr = "|", nt = String.fromCharCode(8214), str = String.fromCharCode(822), str0 = String.fromCharCode(821), sp = String.fromCharCode(160), finger = String.fromCharCode(9679), optional = String.fromCharCode(9675);
      let min = 99, max = 0, fret, frets, char, nut = str0, line, lines = [];
    ...
    module2.exports = function plugin(md) {
...
__export(main_exports, {
  default: () => MarkdownChords
});
module.exports = __toCommonJS(main_exports);
var import_markdown_it = __toESM(require_markdown_it());
var import_markdown_it_chords = __toESM(require_markdown_it_chords());
...
var MarkdownChords = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.md = new import_markdown_it.default("commonmark", { breaks: true }).disable(["code", "fence", "html_block", "lheading", "list", "reference", "table", "autolink", "backticks", "html_inline", "linkify"]).use(import_markdown_it_chords.default);
    /**
     ...
     */
    this.parseCodeBlock = (source, el, ctx) => {
  ...
  get textOpacity() {
    if (this.settings.opacity === "faint")
      return 0.5;
    if (this.settings.opacity === "muted")
      return 0.8;
    return 1;
  }
  get textColor() {
    if (this.settings.color === "text")
      return `var(--text-${this.settings.opacity})`;
    if (this.settings.color === "accent")
      return `hsla(var(--accent-h), var(--accent-s), var(--accent-l), ${this.textOpacity})`;
...
var SettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.createEl("p", {
      text: 'To use chords in Markdown, you must create a "song" or "lyrics" code block with the lyrics and chords of the song. For example:'
    });
    containerEl.createEl("pre", {
      text: "```song\n[C]This is the [F]shortest [G]song in the [C]world!\n```"
    });
    containerEl.createEl("p", {
      text: "Please visit the link below for examples illustrating how to structure chords and chord diagrams."
    });
    containerEl.createEl("a", { text: "Markdown Chords documentation", href: "https://dnotes.github.io/markdown-it-chords" });
    containerEl.createEl("hr");
    containerEl.createEl("h2", { text: "Settings" });
    new import_obsidian.Setting(containerEl).setName("Color").setDesc("Pick a color for chord display").addDropdown((c) => c.addOption("text", "text").addOption("accent", "accent").addOption("red", "red").addOption("orange", "orange").addOption("yellow", "yellow").addOption("green", "green").addOption("cyan", "cyan").addOption("blue", "blue").addOption("purple", "purple").addOption("pink", "pink").setValue(this.plugin.settings.color).onChange(async (value) => {
      this.plugin.settings.color = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Opacity").setDesc("Set opacity for chord display").addDropdown((c) => c.addOption("normal", "normal").addOption("muted", "muted").addOption("faint", "faint").setValue(this.plugin.settings.opacity).onChange(async (value) => {
      this.plugin.settings.opacity = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Chord name size (%)").setDesc("The font size of chord names, as a percentage.").addSlider((c) => c.setLimits(40, 120, 10).setValue(this.plugin.settings.textSize).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.textSize = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Bold").setDesc("Bold chord names").addToggle((c) => c.setValue(this.plugin.settings.bold).onChange(async (value) => {
      this.plugin.settings.bold = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Chord diagram size (px)").setDesc("The font size of chord diagrams, in pixels.").addSlider((c) => c.setLimits(9, 15, 1).setValue(this.plugin.settings.diagramSize).setDynamicTooltip().onChange(async (value) => {
